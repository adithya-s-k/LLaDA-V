...
class DataArguments:
    ...
    image_folder_2: Optional[str] = field(default=None, metadata={"help": "Path to the second image folder, used as a fallback."})
...
def preprocess_llama3(
    ...
    # Reset LLaMA3 chat templates so that it won't include assistant message every time we apply
...
class WebDatasetSupervisedDataset(torch.utils.data.IterableDataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        super(WebDatasetSupervisedDataset, self).__init__()
        self.tokenizer = tokenizer
        self.data_args = data_args
        self.data_path = data_path
        
        # WebDataset related configuration
        self.shuffle_buffer_size = getattr(data_args, "shuffle_buffer_size", 1000)
        self.use_webdataset = getattr(data_args, "use_webdataset", False)
        rank0_print(f"use_webdataset: {self.use_webdataset}")
        rank0_print(f"Initializing WebDataset, data path: {data_path}")
        self.total_samples = getattr(data_args,"total_samples", None)
        rank0_print(f"total_samples: {self.total_samples}")
        
    def __iter__(self):
        # Build WebDataset processing pipeline, optimized for single epoch
        dataset = (
            wds.WebDataset(
                self.data_path, 
                resampled=True,
            ...
            .decode("pilrgb", handler=wds.warn_and_continue) # Equivalent to Image.open(io.BytesIO(data)).convert("RGB")
            .map(self.process_sample)
            .select(lambda x: x is not None) # Filter out None
        )
    ...
    def _calculate_samples(self):
        if self.total_samples:
            if torch.distributed.is_initialized():
                ...
                return math.ceil(self.total_samples / world_size)
            return self.total_samples
        return 0

    def __len__(self):
        return self._calculate_samples()
    ...
    def process_image(self, image, overwrite_image_aspect_ratio=None):
        ...
        try:
            ...
            print(f"Failed to open image. Exception:", exn)
            raise exn
    ...
    def process_sample(self, sample):
        """Process WebDataset sample, return format compatible with LazySupervisedDataset"""
        try:
            # Parse JSON data
            json_data = sample["json"] #json.loads(sample["json"])
            conversations = json_data.get("conversations", [])
            
            # Check if there is image data
            has_image = "jpg" in sample
            image = None
            
            # Only process image if sample contains image
            if has_image:
                image_file = os.path.basename(json_data.get("image", "")) # Is a string or list of strings
                image = sample["jpg"] # One or more images, if multiple images, save as list

                if type(image_file) is list:
                    # Handling multi images
                    if len(image_file) > 1:
                        image = [self.process_image(f, "pad") for f in image]
                        image = [[im[0], im[1], "image"] for im in image]
                    else: 
                        image = [self.process_image(f) for f in image]
                else:
            ...
            # Construct return result in the same format as LazySupervisedDataset
            result = {
                "input_ids": data_dict["input_ids"][0],
                "labels": data_dict["labels"][0],
            }
            
            # Only add image data if there is an image
            if has_image:
                result["image"] = image
            ...
            # Add the same extra fields
            if "prompt" in data_dict:
                result["prompt"] = data_dict["prompt"]
                
            result["id"] = json_data.get("id", 0)
            
            if conversation_lib.default_conversation.version == "llada_plain":
                result["is_plain"] = True
